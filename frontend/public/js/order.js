...
(function(){
  ...
  function wireCheckout(){
    ...
    // Helper: enable/disable Pay Now based on required inputs and cart state
    function updatePayButtonState() {
      ...
      // Hosted checkout only requires we have basic contact + items; no in-page card readiness required
      const enabled = nameFilled && phoneFilled && hasItems;
      ...
      if (pm) {
        if (!hasItems) pm.textContent = 'Add items to your cart to enable checkout.';
        else if (!nameFilled || !phoneFilled) pm.textContent = 'Please fill in your name and phone number.';
        else pm.textContent = '';
      }
    }
    ...
    checkoutForm.addEventListener('submit', async (e) => {
      ...
      // Build an order payload that the server's checkout service expects
      const orderPayload = {
        items: cartItems,
