...
BACKEND_START_CMD = os.getenv("BACKEND_START_CMD", "node api/server_example.js")
...
def wait_for_backend(timeout=15):
    """Wait until backend health endpoint responds 200 or raise RuntimeError."""
    try:
        import requests
    except Exception:
        raise RuntimeError("The 'requests' library is required to check backend health. Install with: pip install requests")
...
def main():
    ...
    # Start ngrok after backend is running
    ...
    try:
        with socketserver.TCPServer(("", PORT), Handler) as httpd:
            ...
            if ngrok_url:
                print(f"Public site (ngrok): {ngrok_url}/order.html")
            httpd.serve_forever()
